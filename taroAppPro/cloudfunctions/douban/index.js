...
async function getDouban (isbn) {
  ...
  // 评论
  let comments = [];
  $('#comments .comment').each((index, value) => {
    comments.push({
      author: $(value).find('.comment-info a').text(),
      content: $(value).find('.comment-content').text(),
      date: $(value).find('.comment-info span').eq(1).text()
    })
  })
